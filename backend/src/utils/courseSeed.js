...
const seedCourses = async () => {
  try {
    const checkQuery = await pool.query('SELECT count(*) FROM courses');
    const count = parseInt(checkQuery.rows[0].count);

    if (count > 0) {
      console.log('Cursos já existem no banco. Pulei a criação.');
      return;
    }

    console.log('Nenhum curso encontrado. Inserindo cursos padrão...');
    ...
    // --- BASE DE PÚBLICO-ALVO (Igual para os dois) ---
    const publicoAlvo = [
      "Iniciantes e Profissionais de todas as áreas que buscam por uma formação ampla em Matemática Financeira e Estatística Básica, com conhecimentos que serão aplicados em momentos oportunos da vida prática.",
      "Iniciantes e Profissionais da área que desejam atualizar seus conhecimentos com professores de referência no mercado.",
      "Iniciantes e profissionais que buscam ferramentas técnicas de planejamento e gestão financeira para seus negócios, incluindo princípios básicos de análises macroeconômicas, gestão de capital, riscos e investimentos.",
      "Interessados em uma metodologia baseada nas trocas e interações entre alunos e profissionais do mercado participantes do curso."
    ];

    // --- CURSO 1: Matemática Financeira ---
    await pool.query(query, [
      'Matemática Financeira',
      40,
      800.00, // VALOR ATUALIZADO
      'À vista ou em duas parcelas mensais de R$ 400,00 (parcelado no cartão).', // PAGAMENTO ATUALIZADO
      'Terças e Quintas-feiras das 19 às 21 hs', // HORÁRIO ATUALIZADO
      ...
      JSON.stringify({
        ...
        "target_audience": publicoAlvo, // NOVO CAMPO
        "items": [
    ...
    // --- CURSO 2: Estatística Básica ---
    await pool.query(query, [
      'Estatística Básica',
      50,
      1500.00,
      'À vista ou em 3 parcelas mensais de R$ 500,00 (parcelado no cartão).',
      'Terças e Quintas-feiras das 19 às 21 hs',
